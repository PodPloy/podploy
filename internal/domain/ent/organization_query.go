...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	for name, query := range _q.withNamedRoles {
		if err := _q.loadRoles(ctx, query, nodes,
			...
			func(n *Organization, e *Role) { n.appendNamedRoles(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedMemberships {
		if err := _q.loadMemberships(ctx, query, nodes,
			...
			func(n *Organization, e *OrganizationMember) { n.appendNamedMemberships(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrganizationQuery) WithNamedRoles(name string, opts ...func(*RoleQuery)) *OrganizationQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithNamedMemberships(name string, opts ...func(*OrganizationMemberQuery)) *OrganizationQuery {
	query := (&OrganizationMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
